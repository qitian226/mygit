...
public final class AuthorityUtil {
	...
	private final static String COOKIE_DOMAIN = CacheLoadPropertiesFile.getPropertiesValue("filepath", "cookiedomain");
	/**
	 * 格式：/login;/images/**
	 */
   ...
   public static String encodeUri(HttpServletRequest request, String uri) throws Exception{
    	String sid="";
    	if(request.getAttribute("sid")!=null){
    		sid=request.getAttribute("sid").toString();
    	}
    	return "analysis?sid="+sid+"&url="+URLEncoder.encode(uri, "UTF-8");
   }
   ...
   public static Boolean checkTocken(HttpServletRequest request) throws Exception{
	   ...
	   if(ip==null||cookie_uid==null||cookie_tocken==null){
		...
		try {
			...
			cookie_tocken=URLDecoder.decode(cookie_tocken, "UTF-8");
	 ...
	 int length=cookies.length;
	   ...
	   if(null!=cookie_tocken && null!=cookie_uid){
		   try {
				...
				String uuid=des.decrypt(cookie_uid);
				request.setAttribute("uid_", uuid);//传递UUID
				if(HashCodeUtil.validatePassword(uuid+"@"+ip,URLDecoder.decode(cookie_tocken,"UTF-8"))){
					   return true; //合法
				   }else{
					   return false;
				   }
