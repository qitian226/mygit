...
var imghostPath="http://img.zuimei.com:8088/";
...
var showFullImgBox = function(url_) {	
	var index=layer.open({
		...
		area : [ '1280px', '90%' ],
		anim : 1,
		content : [ localhostPath + "entry/fullimg?url="+url_, 'no' ], //iframe的url，no代表不显示滚动条
		end : function() { //此处用于演示 
		},
		success : function(layero, index) {
...
$(function() {
	if(getCookie("uid_")!=null){
         ...
         if(null==vlevel || 0==vlevel){
        	 html ="<li><a onclick=\"showUserSet()\"  class=\"set\">会员设置</a></li>" +
		 		...
		 		"<li><a   class=\"user\"  target=\"_blank\" href=\""+localhostPath+"u"+u+"\">"+nname+"</a></li>";
         }else{
		     html="<li><a onclick=\"showUserSet()\"  class=\"set\">会员设置</a></li>" +
		 		"<li class=\"spacer\"><a></a></li>" +
		 		"<li><a onclick=\"showEmailBox()\" class=\"email_p\">私信</a></li>" +
		 		...
		 		"<a  class=\"user\" target=\"_blank\" href=\""+localhostPath+"u"+u+"\">"+nname+"</a></li>";
             }
		 $("#llogin").html(html);
     }else{
  ...
  if($("#more_nav")){
	  var timeOut = null;
	 	var Z_SubList=$("#more_nav_panel");
		$('#more_nav').hover(function(e) {
		...
		var showSubList = function(thisli) {
			...
			$(Z_SubList).css({top: top, opacity: 0}).show().stop().animate({opacity : 1});
		};
