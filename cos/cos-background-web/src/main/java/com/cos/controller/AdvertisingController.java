...
public class AdvertisingController extends BaseController {
	...
	public String delImg(Long id){
	 ...
	 @RequestMapping(value = "/uploadindexfile",produces = "text/plain;charset=UTF-8")  
	 	@ResponseBody
	    public String fileUploadSimpleFile(HttpServletRequest request,HttpServletResponse response) throws MyException, Exception {
		   ...
		   //创建一个通用的多部分解析器  
	        ...
	        if(multipartResolver.isMultipart(request)){ 
	            ...
	            while(iter.hasNext()){  
	                ...
	                if(file != null){
	                	imgurl=FastDfsUtil.uploadFiles(file.getBytes(), "jpg");
	                }
	            }  
