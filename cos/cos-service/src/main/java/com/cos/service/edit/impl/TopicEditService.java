...
public class TopicEditService implements ITopicEditService {
	...
	@Override
	public String addExtendTopic(Long topicId,String title, String desc, String type,String tag,Long accountId,String istop,String smodel,Integer width,Integer height,String data) throws Exception {
		TopicExample exa=new TopicExample();
		exa.createCriteria().andIdEqualTo(topicId);
		Long count=topicDao.countByExample(exa);
		if(count.compareTo(1L)>=0){ //防重
		...
		String imgurl=fileUploadService.uploadSimplePic(topicId, data,width,height,"jpg");
	...
	@Override
	public String addExtendEntry(Long entryId, String title,String desc, Long topicId, Long accountId,
			Integer width, Integer height, String imgdata) throws Exception {
		String imgurl=fileUploadService.uploadSimplePic(entryId, imgdata,width,height,"jpg");
		return addEntry(entryId,title,imgurl,desc,topicId,accountId,width,height);
	}
	@Override
	...
	public String updateEntry(String title, String imgurl, String desc, Long entryId,Long accountId,Integer width, Integer height) {
		...
		if(StringUtils.isNotBlank(entry.getImgUrl())) {
			...
			if(ids.length==2) {
				 FastDfsUtil.delFile(ids[0]);
				 FastDfsUtil.delFile(ids[1]);
			}
			} catch (Exception e) {
